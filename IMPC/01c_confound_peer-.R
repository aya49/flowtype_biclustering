...
#for (ci in 4:1) {
  ...
  for (feat_type in feat_types) { cat("\n  ", feat_type, ": ") #for each type of feature
    ...
    meta_file = meta_file0[match(meta_file0[,id_col],rownames(m0)),]
    
    if (dateRestriction[1]!="") {
      dateind = ymd(meta_file$date) > ymd(dateRestriction[2]) & ymd(meta_file$date) < ymd(dateRestriction[3]) & m0[,5] < 250000 & m0[,5] > 75000 #set according to graphs
      m0 = m0[dateind,]
      meta_file = meta_file[dateind,]
    }
    
      #if wildtype only, clip meta_file and feature matrix
      if (wtonly!="") {
        wtind = grepl(controlL,meta_file[,target_col])
        m0 = m0[wtind,]
        meta_file = meta_file[wtind,!colnames(meta_file)%in%target_col]
      }
      ...
      #for (layer in layers) { cat("layer ",layer,", ")
        for (le in layerequal) { cat("... ")
          ...
          # trim feature matrix
          lei = ""; if (le) lei = "_only"
          mm = trimMatrix(m0,TRIM=T, mc=mc, sampleMeta=meta_file, sampleMeta_to_m1_col=id_col, target_col=target_col, control=control, order_cols=order_cols, colsplitlen=NULL, k=layer, konly=le, countThres=countThres, goodcount=good_count, good_sample=good_sample)
          if (is.null(mm)) next
          Y = mm$m
          meta_file_ordered = mm$sm

          # keep only needed columns in meta_file_ordered_colonly (interested_cols)
          meta_file_ordered_colonly = meta_file_ordered[,which(colnames(meta_file_ordered)%in%interested_cols)]
          interested_colnames = colnames(meta_file_ordered_colonly)
          ...
          for (interested_cov_col in interested_covs_cols) { cat(", cov")
            ...
            if (!is.null(interested_cov_col) | compare_only) {
              ...
              if (length(col_ind)>0) {
                covs1 = meta_file_ordered_colonly[,col_ind]
                ...
                colnames(covs) = colnames(meta_file_ordered_colonly)[col_ind] 
                noCov = F
              }
            }
            ...
            fname0 = paste0(peer_dir, "/peer_", feat_type,"_layer", str_pad(layer, 2, pad = "0"),"-",lei,wtonly,dateRestriction[1],"_n.",ncol(Y))
            ...
            for (factori in 1:length(modelsfactors)) {
              ...
              if (compare_only) {
                ...
                wtind = grepl(controlL,meta_file_ordered[,target_col])
                ...
                for (ai in 1:length(AlphaPlot)) { #for each known/learned factor
                  for (ploti in 1:2) {
                    ...
                    if (ploti==2 & wtonly=="" & !compare_only) {
                      ...
                      Xtemp0 = X
                      wtind = grepl(controlL,meta_file_ordered0[,target_col])
